...
@swagger_auto_schema(method='POST', request_body=ResourceAddSerializer)
@api_view(["POST"])
@permission_classes([IsModerator])
...
@api_view(["GET"])
def get_resource_image(request, resource_id):
    if not Resource.objects.filter(pk=resource_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    resource = Resource.objects.get(pk=resource_id)

    return HttpResponse(resource.image, content_type="image/png")